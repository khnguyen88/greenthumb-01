...
namespace AgenticGreenthumbApi.Factory
{
    public class AgentFactory
    {
        ...
        public static async Task<AgentFactory> CreateAsync(IConfiguration config, KernelFactory kernelFactory, AgentRegistry agentRegistry)
        {
            var agentFactory = new AgentFactory(config, kernelFactory, agentRegistry);
        ...
        public async Task InitializeAsync()
        {
            ...
            List<AgentConfigTemplate> agentTemplates =
                await FileHelper.GetTemplateFilesAsync<AgentConfigTemplate>(agentTemplateSubdirectories);

            foreach (var template in agentTemplates)
            {
                await BuildAgentAsync(template);
            }
        }
        ...
        private async Task BuildAgentAsync(AgentConfigTemplate template)
        {
            try
            {
                ...
                List<KernelFunction> kernelFunctions = new();
                ...
                if (kernelTools.KernelPlugins.Count > 0 || kernelTools.KernelFunctions.Count > 0)
                {
                    ...
                    {
                        foreach (var f in kernelTools.KernelFunctions)
                        {
                            if (kernel.Plugins.TryGetFunction(f.PluginName, f.FunctionName, out var func))
                                kernelFunctions.Add(func);
                        }
                    }
                    else
                    {
                ...
                if (template.Context.Count > 0)
                {
                    instructions += "\n\nHere are some additional context for the instruction:";

                    foreach (var c in template.Context)
                    {
                        string? context =
                            await FileHelper.GetFileFromDirectoryAsync(c.FileDirectory, c.FileName);
            ...
            {
                Console.WriteLine($"Attempts to generate agent failed due to: {ex.Message}");
            }
        }
